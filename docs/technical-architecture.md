...
│   └── web/       # SvelteKit 控制台
├── packages/
│   └── protocol/  # API / Agent / Gateway 共享协议
├── docs/
└── deploy/
```
...
- Runtime 基础信息
- Session 列表
- 当前 Session 的线性消息 timeline
- 在某条 Message 上触发 fork
- 跳转 Session Graph 页面

## 10.2 Session Graph 页面

Session Graph 使用单独页面：
...
职责：
- 展示 Session lineage
- 展示每个 child session 是从哪条 message fork 出来的
- 提供 Runtime 内部会话关系的可视化解释层
