...
for (const persistent of [false, true]) test(`Runtime retries lease conflict without taking over (${persistent})`, { timeout: 8000 }, async () => {
  ...
  const turn = input();
  try {
    const running = serveRuntime({ url, spaceId: turn.spaceId, cwd: process.cwd(), capabilities: { harnesses: ["pi"], models: [] }, harnesses: {}, token: async () => "fixture", signal: controller.signal, store: new RuntimeSessionStore(turn.spaceId), leaseConflictTimeoutMs: 400,
...
for (const kind of ["missing", "invalid", "changed"]) test(`CLI refuses ${kind} handshake identity without another onReady`, { timeout: 5000 }, async () => {
  ...
  try {
    await serveRuntime({ url, spaceId: turn.spaceId, cwd: process.cwd(), capabilities: { harnesses: ["pi"], models: [] }, harnesses: {}, token: async () => "fixture", signal: controller.signal, store: new RuntimeSessionStore(turn.spaceId), onReady: () => { ready++; } });
