...
import type { AppArtifactManifest, AppGetResponse } from "@neta-art/cohub";
import { downloadApp } from "../src/app-download.js";

const sha256 = (value: string | Buffer) => createHash("sha256").update(value).digest("hex");

function manifestBytes(manifest: AppArtifactManifest) {
...
function singleFileManifest(content: Buffer): AppArtifactManifest {
  return {
...
test("downloadApp restores directory artifacts without content verification", async () => {
  ...
  try {
    const result = await downloadApp(directoryDetail(sha256(encodedManifest)), output, fetcher);
...
test("downloadApp restores a single file", async () => {
  ...
  try {
    const result = await downloadApp(fileDetail(sha256(encodedManifest)), output, fetcher);
...
test("downloadApp does not replace a file created during download", async () => {
  const content = Buffer.from("result\n");
  const manifest = singleFileManifest(content);
  const encodedManifest = manifestBytes(manifest);
  const root = await mkdtemp(join(tmpdir(), "cohub-work-download-"));
  ...
  try {
    await assert.rejects(
      downloadApp(fileDetail(sha256(encodedManifest)), output, fetcher),
...
test("downloadApp does not replace a directory created during download", async () => {
  ...
  try {
    await assert.rejects(
      downloadApp(directoryDetail(sha256(encodedManifest)), output, fetcher),
      /Output already exists/,
    );
...
test("downloadApp restores HTML companion assets as a directory bundle", async () => {
  ...
  try {
    const result = await downloadApp(htmlDetail(sha256(encodedManifest)), output, fetcher);
...
test("downloadApp rejects traversal paths before writing", async () => {
  ...
  try {
    await assert.rejects(
      downloadApp(directoryDetail(sha256(encodedManifest)), join(root, "launch"), memoryFetch(new Map([
        ["https://cdn.test/meta/manifest.json", encodedManifest],
      ]))),
...
test("downloadApp accepts CDN-rewritten content of any type", async () => {
  ...
  const output = join(root, "launch");
  try {
    const result = await downloadApp(directoryDetail(sha256(encodedManifest)), output, memoryFetch(new Map([
      ["https://cdn.test/meta/manifest.json", encodedManifest],
...
test("downloadApp removes staged files after content download failure", async () => {
  ...
  try {
    await assert.rejects(
      downloadApp(directoryDetail(sha256(encodedManifest)), join(root, "launch"), async (input) => {
...
test("downloadApp clearly rejects unsupported app types", async () => {
  const work = { id: "work-3", slug: "preview", latestVersion: 1 };
  const fetcher: typeof fetch = async () => {
    throw new Error("fetch should not run");
  };
  await assert.rejects(
    downloadApp({ work, content: { kind: "port", port: 5173, url: "https://preview.test" } } as AppGetResponse, undefined, fetcher),
    /Port apps/,
  );
  await assert.rejects(
    downloadApp({ work, content: { kind: "board", url: "https://cdn.test/board.json" } } as AppGetResponse, undefined, fetcher),
    /Board apps/,
  );
  await assert.rejects(
    downloadApp({ work, content: null } as AppGetResponse, undefined, fetcher),
