#!/bin/bash
# FS API Dev 环境部署脚本
# 仅服务于 /api/spaces/:id/fs/* 和 /api/spaces/:id/checkpoints/:checkpointId/fs/* 路由
...
API_SECRET_NAME=$(get_value "API_SECRET_NAME")
...
echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   Cohub FS API Prod 环境部署     ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
...
PY
  ...
  sed -i.bak \
    ...
    -e "s|__API_SECRET_NAME__|${API_SECRET_NAME}|g" \
...
echo ""
echo -e "${GREEN}✅ FS API Prod 环境部署完成${NC}"
echo ""
echo "查看状态："
