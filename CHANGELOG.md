...
- **Per-Space App Viewer Grants**: app authorization reworked from per-app to per-space viewer grants ((app, viewer, space) key, 14-day TTL) with live per-request scope re-validation, so disabling an app, revoking a grant, or a role downgrade takes effect immediately. Adds `auth.requestSpace()`/`alwaysAsk` consent flows, new `GET/DELETE /apps/:id/grants` APIs, SDK `apps.listMyGrants`/`apps.revokeMyGrant`, and CLI `cohub apps authorize/grants/revoke`; delegated scheduled-task authorization now resolves live scopes at use time and aborts on forged or expired grants instead of spending quota.
- **Model-Generated Session Titles**: sessions now get concise titles generated by configurable model tasks defined in `.cohub/model-tasks.json` (provider/model/prompt per task), dispatched via a new BullMQ job with message-content projection and fallback-title handling.
- **Shared Model Runtime Package**: new `@cohub/model-runtime` package consolidates the multi-provider pi-ai adapter, image-content normalization, and model-task config parsing previously duplicated across the API and agent, giving both runtimes a single model catalog and streaming surface.
- **Localized Public Surfaces**: landing, pricing, and docs rebuilt as shared localized components (LandingView, PricingView, DocsShell) with full Simplified Chinese routes for docs and pricing, per-locale SEO metadata, and localized sitemap.

### Bug Fixes

- Sandbox WebSocket client no longer enters stale reconnect loops after ownership or connection state changes (generation-tracked runs with abort control).
- Streaming sessions stay pinned to the tail: auto-scroll anchoring now survives incoming tokens and layout changes without jumping.
- Space file uploads declare explicit content-length on presigned PUTs, fixing uploads rejected by providers that require it, with browser-managed headers excluded from XHR.
