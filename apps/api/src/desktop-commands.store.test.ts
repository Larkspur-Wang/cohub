...
import {
  claimDesktopCommand,
  getDesktopCommandKey,
  settleDesktopCommandRecord,
  DesktopCommandStoreError,
...
const record = (overrides: Record<string, unknown> = {}) =>
  ({
    ...
    commandId: "cmd-1",
    status: "pending",
    command: { type: "desktop.open", target: { kind: "app", appId: "w-1" } },
    actorUserId: "user-1",
...
function createFakeRedis(seed?: Record<string, unknown>) {
  ...
  for (const [id, value] of Object.entries(seed ?? {})) {
    store.set(getDesktopCommandKey(id), JSON.stringify(value));
  }
...
const settle = (
  ...
  settleDesktopCommandRecord(client, {
    commandId: "cmd-1",
...
describe("claimDesktopCommand", () => {
  it("claims an unused id with the pending lifetime, and a retry sees the winner", async () => {
    ...
    assert.equal((await claimDesktopCommand(client, record())).claimed, true);

    const retry = await claimDesktopCommand(client, record({ status: "applied" }));
    assert.equal(retry.claimed, false);
    assert.equal(retry.record.status, "pending", "the stored record wins");
    assert.deepEqual(appliedTtls, [String(DESKTOP_COMMAND_PENDING_TTL_SECONDS)]);
  });
  ...
  it("uses the terminal lifetime when a command settles during creation", async () => {
    ...
    assert.equal((await claimDesktopCommand(client, terminal)).claimed, true);
    assert.deepEqual(appliedTtls, [String(DESKTOP_COMMAND_TERMINAL_TTL_SECONDS)]);
  });
});

describe("a malformed store reply", () => {
  const badClient = (reply: unknown): DesktopCommandStoreClient => ({
    async get() {
  ...
  for (const [name, reply] of [
    ...
    it(`rejects ${name} instead of dispatching`, async () => {
      await assert.rejects(
        () => claimDesktopCommand(badClient(reply), record()),
        DesktopCommandStoreError,
      );
    });
  }
...
describe("settleDesktopCommandRecord", () => {
