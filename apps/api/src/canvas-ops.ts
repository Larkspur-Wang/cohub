...
function normalizeOp(op: CanvasSemanticOp): CanvasSemanticOp {
  ...
  if (op.type === "document.patch") {
    if (!isRecord(op.payload.patch)) throw new CanvasServiceError(400, "document.patch requires patch");
    const keys = Object.keys(op.payload.patch);
    if (keys.length !== 1 || keys[0] !== "meta") throw new CanvasServiceError(400, "document.patch only supports meta");
    const meta = op.payload.patch.meta;
    if (meta !== null && !isRecord(meta)) throw new CanvasServiceError(400, "document meta must be an object or null");
    if (jsonBytes(meta) > MAX_JSON_FIELD_BYTES) throw new CanvasServiceError(413, "document meta is too large");
  ...
  if (op.type === "node.delete") {
    ...
    if (reason !== undefined && !reason.trim()) throw new CanvasServiceError(400, "reason must be a non-empty string");
