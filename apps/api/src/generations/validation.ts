...
export function validateGenerationContent(declaration: GenerationDeclaration, content: GenerationContentBlock[]): void {
  ...
  for (const block of content) {
    ...
    if ("source" in block && spec.sources && !spec.sources.includes(block.source.type)) {
      throw new GenerationValidationError(`${block.type} source is not supported by ${declaration.model}: ${block.source.type}`);
    }
  }
  ...
  for (const spec of declaration.content.input) {
    ...
    if (spec.min !== undefined && count < spec.min) throw new GenerationValidationError(`Expected at least ${spec.min} ${spec.type} content block(s)`);
    if (spec.max !== undefined && count > spec.max) throw new GenerationValidationError(`Expected at most ${spec.max} ${spec.type} content block(s)`);
  }
}
  ...
  for (const [key, spec] of Object.entries(specs)) {
    ...
    switch (spec.type) {
      ...
      case "number":
        ...
        if (spec.min !== undefined && value < spec.min) throw new GenerationValidationError(`Parameter ${key} must be >= ${spec.min}`);
        if (spec.max !== undefined && value > spec.max) throw new GenerationValidationError(`Parameter ${key} must be <= ${spec.max}`);
        break;
      case "integer":
...
export function mergeTextBlocks(declaration: GenerationDeclaration, content: GenerationContentBlock[]): string {
