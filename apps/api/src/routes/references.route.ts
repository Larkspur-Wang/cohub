...
/**
 * GET /api/references?source=session:<id>&direction=both&kinds=mention,tool_call&days=30&limit=200
 * Neutral reference listing for a resource. Callers render graphs, lists, or
 * rankings from the rows.
 */
router.get("/", async (c) => {
  ...
  // Incoming references originate in other spaces; gate them by viewability so
  // "who references me" never leaks source ids from private spaces. Memoized in
  // queryReferences per distinct space.
  const canViewSpace = (candidateSpaceId: string) =>
    candidateSpaceId === spaceId
      ? Promise.resolve(true)
      : hasPermission(user, "space.view", { spaceId: candidateSpaceId });

  let rows: Awaited<ReturnType<typeof queryReferences>>;
  try {
    rows = await queryReferences({ ref, direction, kinds, since, limit, canViewSpace });
  } catch (error) {
...
/**
 * GET /api/references/aggregate?space=<id>&groupBy=kind&kinds=...&days=30
 * Grouped counts for a space: distributions, influence, and trends.
 */
router.get("/aggregate", async (c) => {
  ...
  try {
    rows = await aggregateReferences({ spaceId, groupBy, kinds, since });
  } catch (error) {
