...
import {
  getAppPromotionProvider,
  listAppPromotionProviders,
} from "../app-promotion-providers.js";
import {
...
const PUBLIC_EVENT_KEYS = new Set<AppPromotionEventKey>([
  "landing",
  "ready",
  "paywall_viewed",
]);
...
router.get("/:appId/promotions", async (c) => {
  ...
  const promotions = await db
    .select()
    .from(appPromotions)
    .where(eq(appPromotions.appId, appId))
    .orderBy(asc(appPromotions.createdAt));
  return c.json({
    promotions: promotions.map(serializePromotion),
    providers: listAppPromotionProviders(),
  });
});

router.post("/:appId/promotions", async (c) => {
  const user = useAuth(c);
  if (user instanceof Response) return user;
  const appId = c.req.param("appId");
  if (!requireValidId(appId)) return c.json({ message: "app not found" }, 404);
  ...
  if (app.status !== "published" || !app.currentVersionId) {
    return c.json({ message: "publish the app before creating a promotion", code: "app_not_published" }, 409);
  }
  ...
  const provider = getAppPromotionProvider(providerKey);
  ...
  const promotionValues: typeof appPromotions.$inferInsert = {
    appId,
    name,
  ...
  const [promotion] = await db.insert(appPromotions).values(promotionValues).returning();
...
router.get("/:appId/promotions/:promotionId/stats", async (c) => {
  const user = useAuth(c);
  if (user instanceof Response) return user;
  const appId = c.req.param("appId");
  const promotionId = c.req.param("promotionId");
  if (!requireValidId(appId) || !requireValidId(promotionId)) return c.json({ message: "promotion not found" }, 404);
  const [promotion] = await db.select().from(appPromotions).where(and(
    eq(appPromotions.id, promotionId),
    eq(appPromotions.appId, appId),
  )).limit(1);
  ...
  const [totalRows, rows] = await Promise.all([
    db.select({
      eventKey: appPromotionStatsHourly.eventKey,
      eventCount: sql<string>`coalesce(sum(${appPromotionStatsHourly.eventCount}), 0)`,
    }).from(appPromotionStatsHourly)
      .where(eq(appPromotionStatsHourly.promotionId, promotionId))
      .groupBy(appPromotionStatsHourly.eventKey),
    db.select({
      bucketStartAt: appPromotionStatsHourly.bucketStartAt,
      eventKey: appPromotionStatsHourly.eventKey,
      eventCount: appPromotionStatsHourly.eventCount,
    }).from(appPromotionStatsHourly).where(and(
      eq(appPromotionStatsHourly.promotionId, promotionId),
      gte(appPromotionStatsHourly.bucketStartAt, startDay),
    )).orderBy(asc(appPromotionStatsHourly.bucketStartAt)),
  ]);
...
router.post("/:appId/promotions/:promotionId/events", async (c) => {
  const appId = c.req.param("appId");
  const promotionId = c.req.param("promotionId");
  if (!requireValidId(appId) || !requireValidId(promotionId)) return c.json({ message: "promotion not found" }, 404);
  const body = await c.req.json().catch(() => null) as Record<string, unknown> | null;
  ...
  if (!PUBLIC_EVENT_KEYS.has(eventKey as AppPromotionEventKey)) {
    return c.json({ message: "event is invalid" }, 400);
  }
  if (!eventId) return c.json({ message: "eventId is invalid" }, 400);

  const row = await resolvePublishedAppPromotion(appId, promotionId);
  ...
  const browser = recordResolvedAppPromotionEvent(c, {
    promotion: row.promotion,
    appVersionId: row.app.currentVersionId,
...
router.post("/:appId/promotions/:promotionId/registration", async (c) => {
  const user = useAuth(c);
  if (user instanceof Response) return user;
  const appId = c.req.param("appId");
  const promotionId = c.req.param("promotionId");
