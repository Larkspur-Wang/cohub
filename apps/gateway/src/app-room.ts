...
import { authorizeAppRoom } from "./api-client.js";
...
type RoomConnection = {
  ...
  userId?: string;
  token?: string;
  appRooms: Map<string, { participantId: string; ticket: string; room: RealtimeRoomDescriptor }>;
};
...
export type AppRoomPresenceRate = { startedAt: number; count: number };
 ...
 */
export function consumeAppRoomPresenceRate(rate: AppRoomPresenceRate, now = Date.now()): boolean {
...
export type AppRoomMembershipStatus = "active" | "expired" | "revoked" | "superseded";
 ...
 */
export class AppRoomMembershipLostError extends Error {
  constructor(message: string, readonly status: Exclude<AppRoomMembershipStatus, "active">) {
    super(message);
    this.name = "AppRoomMembershipLostError";
  }
}

/** Script status codes that mean this connection no longer holds its seat. */
const MEMBERSHIP_LOST_CODES: Record<number, { message: string; status: Exclude<AppRoomMembershipStatus, "active"> }> = {
...
const membershipError = (code: number) => {
  ...
  if (lost) return new AppRoomMembershipLostError(lost.message, lost.status);
...
export async function authorizeAppRoomJoin(ctx: RoomConnection, input: { roomId: string; ticket: string }) {
  if (!ctx.token || !ctx.userId) throw new Error("authentication required");
  return authorizeAppRoom({ authToken: ctx.token, roomId: input.roomId, ticket: input.ticket });
}

/**
 * Claims the seat only. The caller subscribes to the room next and then reads the
 * snapshot with {@link readAppRoomSnapshot}: an unseated connection must never enter
 ...
 */
export async function claimAppRoomSeat(
  ctx: RoomConnection,
  input: { roomId: string; ticket: string; admission: Awaited<ReturnType<typeof authorizeAppRoomJoin>> },
  ...
  ctx.appRooms.set(input.roomId, { participantId, ticket: input.ticket, room: admission.room });
...
export async function readAppRoomSnapshot(roomId: string, redis: Redis = redisCommandClient) {
...
export async function leaveAppRoom(ctx: RoomConnection, roomId: string, redis: Redis = redisCommandClient) {
 ...
 */
export async function sweepAppRoomLeases(roomId: string, redis: Redis = redisCommandClient) {
  const result = await redis.eval(
    SWEEP_LEASES_SCRIPT,
    2,
    getRealtimeRoomMembersKey(roomId),
...
export async function renewAppRoomMembership(ctx: RoomConnection, roomId: string, redis: Redis = redisCommandClient): Promise<AppRoomMembershipStatus> {
  const membership = ctx.appRooms.get(roomId);
...
export async function updateAppRoomPresence(ctx: RoomConnection, roomId: string, presence: Record<string, unknown> | null, redis: Redis = redisCommandClient) {
  const membership = ctx.appRooms.get(roomId);
...
export async function publishAppRoomEvent(ctx: RoomConnection, input: {
  roomId: string;
  event: string;
  data: unknown;
  clientEventId?: string;
}, redis: Redis = redisCommandClient) {
  const membership = ctx.appRooms.get(input.roomId);
  if (!membership) throw new Error("room membership is required");
...
export async function publishAppRoomControlEvent(input: {
  roomId: string;
  type: string;
