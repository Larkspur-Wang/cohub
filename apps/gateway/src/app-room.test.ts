...
const connection = () => ({
  ...
  userId: "user-1",
  token: "token-1",
  appRooms: new Map([["room-1", { participantId: "participant-1", ticket: "ticket-1", room }]]),
});
...
test("renewing a Work room membership distinguishes expiry from revocation", async () => {
  ...
  for (const [code, expected] of [[-1, "revoked"], [-2, "expired"], [-5, "superseded"]] as const) {
    ...
    assert.equal(await renewAppRoomMembership(ctx, "room-1", evalStub(code)), expected);
    assert.equal(ctx.appRooms.size, 1, "teardown belongs to the Gateway, not here");
  }
});
...
test("an operation that outruns the heartbeat reports why the membership is gone", async () => {
  ...
  for (const [code, status] of [[-1, "revoked"], [-2, "expired"], [-3, "revoked"], [-5, "superseded"]] as const) {
    await assert.rejects(
      () => updateAppRoomPresence(connection(), "room-1", null, evalStub(code)),
      (error: unknown) => error instanceof AppRoomMembershipLostError && error.status === status,
    );
  }

  // A rate limit is not a membership problem; the seat is still held.
  await assert.rejects(
    () => updateAppRoomPresence(connection(), "room-1", null, evalStub(-4)),
    (error: unknown) => !(error instanceof AppRoomMembershipLostError),
  );
});
...
test("Work room presence admission enforces and resets its rate window", () => {
  ...
  assert.equal(consumeAppRoomPresenceRate(rate, 1_500), false, "the window must reject once its budget is spent");
  assert.equal(consumeAppRoomPresenceRate(rate, 2_000), true, "a later window starts fresh");
...
test("sweeping Work room leases announces every claimed participant once", async () => {
  ...
  const removed = await sweepAppRoomLeases("room-1", redis);
...
test("sweeping Work room leases tolerates a room that expired mid-sweep", async () => {
  ...
  assert.deepEqual(await sweepAppRoomLeases("room-1", redis), ["participant-2"]);
});
