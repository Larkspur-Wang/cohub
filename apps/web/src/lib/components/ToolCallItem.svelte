...
<div class="group overflow-hidden rounded-md">
	<button
		type="button"
		class="timeline-row py-0.5 text-left transition-colors hover:bg-bg-hover/50 cursor-pointer rounded-md"
		onclick={toggle}
	>
		...
		<span class="timeline-row-main flex min-w-0 items-center gap-2">
		...
		<span class="timeline-row-trailing text-text-tertiary">
			{#if loading && expanded}
	...
	{#if expanded}
		<div class="timeline-row-nested pr-2 py-0.5 space-y-0.5">
