...
function renewalLabel(item: BillingCatalogProduct) {
	...
	if (item.interval === "monthly") return `${price}/month`;
	if (item.interval === "quarterly") return `${price}/quarter`;
	if (item.interval === "yearly") return `${price}/year`;
	return price;
}
			...
			<header class="flex shrink-0 items-start justify-between gap-4 border-b border-border-subtle px-5 py-4">
				<div class="min-w-0">
					...
					<p class="mt-0.5 text-[11px] text-text-tertiary">
						{product.kind === "plan" ? "Subscription checkout" : "Credit package checkout"}
					</p>
				</div>
				<button type="button" onclick={close} class="flex h-9 w-9 shrink-0 items-center justify-center rounded-[5px] text-text-tertiary transition-colors hover:bg-bg-hover hover:text-text-primary" title="Close" aria-label="Close checkout">
					<X class="h-4 w-4" />
				</button>
			</header>
			...
			<div class="min-h-0 flex-1 overflow-y-auto px-5 py-5 pb-safe">
				{#if automaticOffer && !hasAppliedCode}
					<div class="mb-5 flex items-center justify-between gap-3 border-b border-border-subtle pb-4">
						<div class="min-w-0">
							<div class="text-[12px] font-medium text-text-primary">First purchase offer</div>
							<div class="mt-0.5 truncate text-[11px] text-text-tertiary">{automaticOffer.name}</div>
						</div>
						<span class="shrink-0 rounded-[4px] bg-brand/12 px-2 py-1 text-[11px] font-semibold text-brand">50% off</span>
					</div>
				{/if}
				...
				<div class="space-y-2.5 text-[12px]">
					...
					{#if discountAmount > 0}
						<div class="flex items-center justify-between gap-4 text-text-tertiary">
							<span>{hasAppliedCode ? "Promo code" : "First purchase discount"}</span>
					...
					<div class="flex items-baseline justify-between gap-4 border-t border-border-subtle pt-3">
						<span class="font-medium text-text-primary">Due today</span>
				...
				<form class="mt-6" onsubmit={applyPromotionCode}>
					...
					<div class="flex gap-2">
						<input id="billing-promotion-code" bind:value={promotionCode} disabled={previewLoading || checkoutLoading || hasAppliedCode} maxlength="256" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="Enter code" class="h-11 min-w-0 flex-1 rounded-[5px] border border-border-subtle bg-bg-input px-3 font-mono text-[13px] uppercase text-text-primary outline-none transition-colors placeholder:normal-case placeholder:text-text-placeholder focus:border-brand disabled:opacity-60" />
						{#if hasAppliedCode}
							<button type="button" onclick={removePromotionCode} disabled={checkoutLoading} class="flex h-11 w-11 items-center justify-center rounded-[5px] border border-border-subtle text-text-tertiary transition-colors hover:bg-bg-hover hover:text-text-primary" title="Remove promo code" aria-label="Remove promo code">
								<X class="h-4 w-4" />
							</button>
						{:else}
							<button type="submit" disabled={!promotionCode.trim() || previewLoading || checkoutLoading} class="inline-flex h-11 min-w-20 items-center justify-center rounded-[5px] border border-border-subtle bg-bg-input px-3 text-[12px] font-medium text-text-primary transition-colors hover:bg-bg-hover disabled:cursor-not-allowed disabled:opacity-50">
								{#if previewLoading}<Loader2 class="h-4 w-4 animate-spin" />{:else}Apply{/if}
							</button>
						{/if}
					</div>
					{#if hasAppliedCode}
						<p class="mt-2 flex items-center gap-1.5 text-[11px] text-success-soft"><Check class="h-3.5 w-3.5" /> Code applied</p>
					{/if}
				</form>
