...
function colorHex(id: string, mode: "light" | "dark" = "dark"): string {
	const color = resolveBoardColor(id, mode);
	return `#${color.stroke.toString(16).padStart(6, "0")}`;
}

function transformAttr(frame: BoardFrame): string {
...
function exportDraw(item: BoardDrawItem): string {
	const stroke = colorHex(item.color);
...
function exportArrow(item: BoardArrowItem, getFrame: FrameLookup): string {
...
function exportFrame(item: BoardItem): string {
...
export function itemToSvg(item: BoardItem, getFrame: FrameLookup): string {
	switch (item.type) {
		case "text":
