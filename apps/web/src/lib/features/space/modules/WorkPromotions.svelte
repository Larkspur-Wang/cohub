...
async function loadPromotions() {
	...
	error = "";
	try {
		const result = await sdk.works.listPromotions(workId);
...
async function loadStats(promotionId: string) {
	statsLoading = true;
	try {
		stats = await sdk.works.getPromotionStats(workId, promotionId);
	} catch {
		stats = null;
	} finally {
		statsLoading = false;
...
async function createPromotion(event: SubmitEvent) {
	...
	try {
		...
		const result = await sdk.works.createPromotion(workId, {
			name: name.trim(),
			provider,
			parameters,
		});
...
<section class="border-b border-border-subtle/70 pb-5" aria-labelledby="work-promotions-heading">
	<div class="mb-4 flex items-center justify-between gap-3">
		<div class="flex items-center gap-2">
			<BarChart3 class="h-3.5 w-3.5 text-text-tertiary" />
			<h2 id="work-promotions-heading" class="text-[10px] font-medium uppercase tracking-[0.18em] text-text-placeholder">Promotions</h2>
		</div>
		<button
			type="button"
