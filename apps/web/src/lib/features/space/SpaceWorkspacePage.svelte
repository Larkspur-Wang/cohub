...
function openCronjobModelSelector(mode: "new" | "edit") {
	...
	if (mode === "new" && !cronjobNewModel && firstCatalogModel) {
...
async function loadCronjobRuns(options: { reset?: boolean } = {}) {
	if (!cronjobDetail || !routeCronjobId) return;
	...
	try {
		...
		if (routeView !== "cronjob" || routeCronjobId !== requestCronjobId) return;
		cronjobRuns = reset
			? runs
			: [
					...cronjobRuns,
		...
		mergeCachedCronJobTaskRuns(requestSpaceId, requestCronjobId, runs);
	} catch (error) {
		cronjobRunsError =
...
async function handleCreateCronjobSubmit(event: SubmitEvent) {
	...
	try {
		const response = await sdk.space(spaceId).prompt({
			...
			...(cronjobNewModel
				? { provider: cronjobNewModel.provider, model: cronjobNewModel.id }
				: {}),
			schedule: {
				mode: "repeat",
...
$effect(() => {
	if (routeView === "cronjob" && routeCronjobId) {
		cronjobRuns = [];
