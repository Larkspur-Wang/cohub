...
type Ref = { kind: "file" | "board" | "port" | "work"; key: string };
...
function createHarness() {
	...
	const controller = createPreviewWorkspaceController({
		...
		getWorkTabs: () => workIds.map((workId) => ({ workId, loading: false })),
		...
		openWork: (input) => {
			if (!workIds.includes(input.workId)) workIds = [...workIds, input.workId];
		...
		closeWork: (workId) => {
			if (!workId) return;
			workIds = drop(workIds, workId);
			if (activeWorkId === workId) activeWorkId = workIds.at(-1) ?? null;
		},
...
test("a context teardown never writes the URL of the route it is leaving for", async () => {
	...
	controller.resetForContext(() => {
		controller.tabClosed("work", WORK_ID);
...
test("every domain reports closed tabs to the coordinator", () => {
	...
	for (const [file, callback] of [
		...
		["board-preview-controller.svelte.ts", "onBoardClosed"],
		["port-preview-controller.svelte.ts", "onPortClosed"],
		["work-preview-controller.svelte.ts", "onWorkClosed"],
	] as const) {
...
test("panels open only once their tab is the committed active surface", () => {
	...
	const board = readFileSync(
		new URL("board-preview-controller.svelte.ts", modules),
		"utf8",
	);
	const port = readFileSync(
		new URL("port-preview-controller.svelte.ts", modules),
		"utf8",
	);
	const work = readFileSync(
		new URL("work-preview-controller.svelte.ts", modules),
		"utf8",
	);
	...
	for (const [source, activeAssignment] of [
		...
		[work, "activeWorkId = input.workId"],
	] as const) {
