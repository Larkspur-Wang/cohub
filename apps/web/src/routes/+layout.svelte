<script lang="ts">
  ...
  // Reactive derivation for active tab logic using $page from 'svelte/store' or new state
      ...
      {#if $token}
        ...
        <div class="flex items-center gap-4">
          <div class="w-10 h-10 rounded-full bg-gray-100 border-2 border-white shadow-sm overflow-hidden cursor-pointer hover:border-brand transition-colors" title="Settings">
            <img src="https://api.dicebear.com/7.x/notionists/svg?seed={$token.slice(0,5)}" alt="avatar" class="w-full h-full object-cover" />
          </div>
          <button 
