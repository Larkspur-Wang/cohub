...
<div class="flex min-h-0 flex-1 flex-col overflow-hidden bg-bg-primary">
	...
	<main class="min-h-0 flex-1 overflow-y-auto px-3 py-6 sm:px-5 sm:py-8">
			...
			{:else}
				...
				<section class="space-y-4">
					<div class="flex items-center gap-2.5">
						...
						<div>
							<div class="text-[14px] font-medium text-text-primary">Profile</div>
					...
					<div class="flex flex-col gap-4 sm:flex-row sm:items-start">
						...
						<div class="min-w-0 flex-1 space-y-4">
							...
							<div class="grid gap-4 sm:grid-cols-2">
								<div>
									<div class="mb-1 text-[11px] text-text-tertiary">Space ID</div>
								...
								<div>
									<div class="mb-1 text-[11px] text-text-tertiary">Public URL</div>
							...
							<label class="block">
								<div class="mb-1.5 text-[11px] text-text-tertiary">Description</div>
				...
				<section class="space-y-4">
					<div class="flex items-center justify-between gap-3">
						<div class="flex items-center gap-2.5">
							...
							<div>
								<div class="text-[14px] font-medium text-text-primary">Access</div>
								<div class="text-[12px] text-text-tertiary">Members, permissions, invites.</div>
							</div>
						</div>
						<button type="button" onclick={() => { showInvitePanel = true; inviteCreateError = ""; }} class="inline-flex min-h-8 shrink-0 items-center justify-center gap-1.5 rounded-[6px] border border-brand/20 bg-brand-bg px-3 py-2 text-[12px] font-medium text-brand-muted-fg transition-colors hover:bg-brand-muted"><Link class="h-3.5 w-3.5" /> Invite</button>
					</div>

					<div class="space-y-5">
						<div class="grid gap-4 sm:grid-cols-2">
							<label class="block">
								<span class="mb-1.5 block text-[11px] text-text-tertiary">Signed-in users</span>
								<select value={access?.signed_in_user ?? ""} disabled={!canManageSpaceMembers} onchange={(e) => { const value = (e.currentTarget as HTMLSelectElement).value as SpaceRole | ""; void setAccess({ signed_in_user: value || null }); }} class="w-full rounded-[6px] border border-border-subtle bg-bg-input px-2.5 py-2 text-[12px] text-text-primary focus:border-brand/40 focus:outline-none disabled:opacity-60"><option value="">None</option><option value="guest">Guest</option><option value="builder">Builder</option></select>
							</label>
							<label class="block">
								<span class="mb-1.5 block text-[11px] text-text-tertiary">Anonymous</span>
								<select value={access?.anonymous_user ?? ""} disabled={!canManageSpaceMembers} onchange={(e) => { const value = (e.currentTarget as HTMLSelectElement).value as SpaceRole | ""; void setAccess({ anonymous_user: value || null }); }} class="w-full rounded-[6px] border border-border-subtle bg-bg-input px-2.5 py-2 text-[12px] text-text-primary focus:border-brand/40 focus:outline-none disabled:opacity-60"><option value="">None</option><option value="guest">Guest</option></select>
							</label>
						</div>
						...
						<div class="space-y-3">
							...
							<div class="space-y-1.5">
								{#each members as member (member.userId)}
									<div class="group grid grid-cols-[auto_1fr] gap-2 rounded-[8px] bg-bg-content px-3 py-2 sm:flex sm:items-center">
								...
								{:else}
									<div class="rounded-[8px] bg-bg-content px-3 py-2 text-[12px] text-text-tertiary">No members.</div>
								{/each}
							</div>
						</div>
						...
						<div class="border-t border-border-subtle pt-4">
							...
							{:else if invitations.length === 0}
								<div class="rounded-[8px] bg-bg-content px-3 py-2 text-[12px] text-text-tertiary">No invite links.</div>
							{:else}
								<div class="space-y-1.5">
									{#each invitations as invitation (invitation.token)}
										<div class="rounded-[8px] bg-bg-content px-3 py-2">
											<div class="flex items-center justify-between gap-3">
				...
				<section class="space-y-4">
					<div class="flex items-center gap-2.5">
						...
						<div>
							<div class="text-[14px] font-medium text-text-primary">Runtime inputs</div>
							<div class="text-[12px] text-text-tertiary">Env vars and mounted spaces.</div>
						</div>
					</div>
					<div class="space-y-6">
						<div class="space-y-3">
						...
						<div class="border-t border-border-subtle pt-5 space-y-3">
							...
							{#if shouldShowBaseModRecommendation && recommendedBaseMod}
								<div class="flex flex-col gap-2 rounded-[8px] bg-bg-content px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
									<div class="min-w-0">
										<div class="flex flex-wrap items-center gap-2">
				...
				<section class="space-y-4">
					<div class="flex items-center gap-2.5">
						...
						<div>
							<div class="text-[14px] font-medium text-text-primary">Channels</div>
							<div class="text-[12px] text-text-tertiary">External channel bindings.</div>
						</div>
					</div>
					<div class="space-y-3">
						...
						<div class="space-y-1.5">{#each channels as binding (binding.id)}<div class="space-y-2 rounded-[8px] bg-bg-content px-3 py-2"><div class="flex items-center justify-between gap-3"><span class="min-w-0 truncate text-[12px] text-text-secondary">{binding.channel?.provider ?? 'channel'} · {binding.channel?.name ?? binding.channelId}</span><button type="button" onclick={() => unbindChannel(binding.channelId)} disabled={!canManageSpaceChannels} class="shrink-0 text-[11px] text-text-placeholder hover:text-error-soft disabled:opacity-50">Unbind</button></div><ChannelModelPicker model={binding.config?.model ?? null} disabled={!canManageSpaceChannels} saving={savingChannelConfigIds.has(binding.id)} onSelect={(model) => saveChannelModel(binding, model)} /></div>{:else}<div class="rounded-[8px] bg-bg-content px-3 py-2 text-[12px] text-text-tertiary">No bound channels.</div>{/each}</div>
					</div>
				</section>
				...
				<section class="space-y-4">
					<div class="flex items-center justify-between gap-3">
						<div class="flex items-center gap-2.5">
							...
							<div>
								<div class="text-[14px] font-medium text-text-primary">Sandbox</div>
								<div class="text-[12px] text-text-tertiary">Policy, health, runtime image.</div>
							</div>
						</div>
						<button type="button" onclick={forceRecoverSandbox} disabled={!canManageSpaceSandbox || recoveringSandbox} class="inline-flex min-h-8 shrink-0 items-center justify-center gap-1.5 rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 text-[12px] text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-primary disabled:opacity-50">{#if recoveringSandbox}<Loader2 class="h-3.5 w-3.5 animate-spin" /> Recovering{:else}<RefreshCw class="h-3.5 w-3.5" /> Force recover{/if}</button>
					</div>
					...
					<div class="space-y-5">
						...
						<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
							<div class="min-w-0">
								<div class="text-[11px] text-text-tertiary">Compute spec</div>
								<div class="mt-1 flex flex-wrap items-center gap-2">
									<span class="text-[14px] font-semibold text-text-primary">{getSandboxSpecLabel(sandboxSpec)}</span>
									<span class="font-mono text-[12px] text-text-tertiary">{getSandboxSpecSummary(sandboxSpec)}</span>
									{#if appliedSandboxSpec && appliedSandboxSpec !== sandboxSpec}<span class="rounded-full bg-warning-bg px-2 py-0.5 text-[11px] font-medium text-warning-soft">Restart pending</span>{/if}
								</div>
							</div>
							<button type="button" onclick={() => (specPickerOpen = true)} disabled={!canManageSpaceSandbox || savingSandboxSpec} class="inline-flex min-h-9 shrink-0 items-center justify-center gap-1.5 rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 text-[12px] font-medium text-text-secondary transition-colors hover:bg-bg-hover hover:text-text-primary disabled:opacity-50"><Zap class="h-3.5 w-3.5" />{savingSandboxSpec ? "Saving…" : "Choose spec"}</button>
						</div>

						<!-- Hibernate policy (flat) -->
						<div class="flex flex-col gap-3 border-t border-border-subtle pt-5 sm:flex-row sm:items-start sm:justify-between">
							<div>
								<div class="text-[11px] text-text-tertiary">Hibernate policy</div>
								<div class="mt-1 text-[12px] text-text-secondary">Current: {sandboxAutoDestroyMode === "never" ? "Never" : formatTtl(sandboxIdleTtlSeconds)}</div>
							</div>
							<div class="flex flex-col gap-2 sm:items-end">
								...
								<button type="button" onclick={saveSandboxConfig} disabled={!canManageSpaceSandbox || savingSandboxConfig} class="inline-flex min-h-8 items-center justify-center rounded-[6px] bg-brand px-3 py-2 text-[12px] font-medium text-brand-contrast-fg hover:bg-brand-hover disabled:opacity-50">{savingSandboxConfig ? "Saving…" : "Save policy"}</button>
							</div>
						</div>
						...
						<!-- Health -->
						<div class="grid gap-3 border-t border-border-subtle pt-5 lg:grid-cols-[1.15fr_0.85fr]">
							<div class="rounded-[8px] bg-bg-content p-3.5">
								<div class="flex flex-wrap items-center gap-2">
								...
								<div class="mt-4 grid grid-cols-1 gap-3 text-[12px] sm:grid-cols-2">
									<div title={getSandboxActivityTitle()}><div class="text-[10px] uppercase tracking-[0.14em] text-text-placeholder">Activity</div><div class="mt-1 text-text-primary">{getSandboxActivityText()}</div></div>
									<div title={getSandboxHeartbeatTitle()}><div class="text-[10px] uppercase tracking-[0.14em] text-text-placeholder">Heartbeat</div><div class="mt-1 text-text-primary">{formatRelativeTime(sandbox?.lastHeartbeatAt)}</div></div>
									<div><div class="text-[10px] uppercase tracking-[0.14em] text-text-placeholder">Stopped at</div><div class="mt-1 text-text-primary">{formatRelativeTime(sandbox?.stoppedAt)}</div><div class="mt-0.5 break-words text-[11px] text-text-placeholder">{formatTime(sandbox?.stoppedAt)}</div></div>
									<div class="min-w-0"><div class="text-[10px] uppercase tracking-[0.14em] text-text-placeholder">Pod</div><div class="mt-1 min-w-0 break-all font-mono text-[11px] text-text-primary">{sandbox?.podName ?? '—'}</div><div class="mt-0.5 break-all text-[11px] text-text-placeholder">{getSandboxMetaValue('podIp') || 'IP unavailable'}</div></div>
								</div>
							</div>
							<div class="grid grid-cols-1 gap-2 text-[12px] sm:grid-cols-2 lg:grid-cols-1">
								<div class="min-w-0 rounded-[8px] bg-bg-content px-3 py-2"><div class="text-text-placeholder">Desired image</div><div class="mt-0.5 min-w-0 break-all font-mono text-[11px] leading-relaxed text-text-primary">{sandbox?.desiredImage ?? '—'}</div></div>
								<div class="min-w-0 rounded-[8px] bg-bg-content px-3 py-2"><div class="text-text-placeholder">Reported image</div><div class="mt-0.5 min-w-0 break-all font-mono text-[11px] leading-relaxed text-text-primary">{(sandbox?.reportedImageVersion ?? getSandboxMetaValue('imageVersion')) || '—'}</div></div>
								<div class="rounded-[8px] bg-bg-content px-3 py-2"><div class="text-text-placeholder">Report refreshed</div><div class="mt-0.5 text-text-primary">{formatRelativeTime(sandbox?.reportedAt)}</div></div>
							</div>
						</div>
						{#if sandboxRecoveryMessage}<div class="rounded-[6px] border border-success-soft/30 bg-success-bg px-3 py-2 text-[12px] text-success-soft">{sandboxRecoveryMessage}</div>{/if}
						{#if sandboxRecoveryError}<div class="rounded-[6px] border border-error-soft/30 bg-error-bg px-3 py-2 text-[12px] text-error-soft break-words">{sandboxRecoveryError}</div>{/if}
					</div>
				</section>
			{/if}
		</div>
	...
	<div class="p-5 pb-safe">
		<div class="mb-4 flex items-start justify-between gap-3">
			<div>
				...
				<p class="mt-1 text-[12px] text-text-tertiary">Choose a role and expiry.</p>
			</div>
			<button type="button" onclick={() => { showInvitePanel = false; }} class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-[5px] text-text-tertiary transition-colors hover:bg-bg-hover hover:text-text-secondary"><X class="h-4 w-4" /></button>
		</div>
		...
		<div class="space-y-3">
			<div><label class="mb-1.5 block text-[10px] font-medium uppercase tracking-wider text-text-tertiary" for="invite-role">Role</label><select id="invite-role" bind:value={inviteRole} class="w-full rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 text-[13px] text-text-primary focus:border-brand/40 focus:outline-none"><option value="builder">Builder</option><option value="guest">Guest</option><option value="host">Host</option></select></div>
			<div><label class="mb-1.5 block text-[10px] font-medium uppercase tracking-wider text-text-tertiary" for="invite-ttl">Valid for</label><select id="invite-ttl" bind:value={inviteTtlDays} class="w-full rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 text-[13px] text-text-primary focus:border-brand/40 focus:outline-none"><option value={1}>1 day</option><option value={7}>7 days</option><option value={14}>14 days</option><option value={30}>30 days</option></select></div>
			<div><label class="mb-1.5 block text-[10px] font-medium uppercase tracking-wider text-text-tertiary" for="invite-max-uses">Max uses <span class="normal-case tracking-normal text-text-placeholder">(0 = unlimited)</span></label><input id="invite-max-uses" type="number" bind:value={inviteMaxUses} min="0" max="10000" step="1" class="w-full rounded-[6px] border border-border-subtle bg-bg-input px-3 py-2 text-[13px] text-text-primary focus:border-brand/40 focus:outline-none" /></div>
		</div>
