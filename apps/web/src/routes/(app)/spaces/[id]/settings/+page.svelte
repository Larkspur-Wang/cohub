...
<Sheet open={showInvitePanel} onClose={() => { showInvitePanel = false; }} maxWidth="400px">
	<div class="p-5 pb-safe">
		...
		<div class="space-y-3">
			<div><label class="mb-1.5 block text-[10px] font-medium uppercase tracking-wider text-text-tertiary" for="invite-role">Role</label><select id="invite-role" bind:value={inviteRole} class="h-9 w-full rounded-[5px] border border-border-subtle bg-bg-input px-3 text-[13px] text-text-primary focus:border-brand/40 focus:outline-none"><option value="builder">Builder</option><option value="guest">Guest</option><option value="host">Host</option></select></div>
			<div><label class="mb-1.5 block text-[10px] font-medium uppercase tracking-wider text-text-tertiary" for="invite-ttl">Valid for</label><select id="invite-ttl" bind:value={inviteTtlDays} class="h-9 w-full rounded-[5px] border border-border-subtle bg-bg-input px-3 text-[13px] text-text-primary focus:border-brand/40 focus:outline-none"><option value={1}>1 day</option><option value={7}>7 days</option><option value={14}>14 days</option><option value={30}>30 days</option></select></div>
			<div><label class="mb-1.5 block text-[10px] font-medium uppercase tracking-wider text-text-tertiary" for="invite-max-uses">Max uses <span class="normal-case tracking-normal text-text-placeholder">(0 = unlimited)</span></label><input id="invite-max-uses" type="number" bind:value={inviteMaxUses} min="0" max="10000" step="1" class="h-9 w-full rounded-[5px] border border-border-subtle bg-bg-input px-3 text-[13px] text-text-primary focus:border-brand/40 focus:outline-none" /></div>
		</div>
		<div class="mt-5 flex justify-end gap-2"><button type="button" onclick={() => { showInvitePanel = false; }} class="h-9 rounded-[5px] border border-border-subtle bg-bg-hover px-4 text-[12px] text-text-tertiary transition-colors hover:bg-bg-hover-strong hover:text-text-secondary">Cancel</button><button type="button" onclick={() => { void createInvite(); }} disabled={creatingInvite} class="inline-flex h-9 items-center gap-1.5 rounded-[5px] bg-brand px-4 text-[12px] font-medium text-brand-contrast-fg transition-colors hover:bg-brand-hover disabled:opacity-50">{#if creatingInvite}<Loader2 class="h-3.5 w-3.5 animate-spin" /> Creating…{:else}<Link class="h-3.5 w-3.5" /> Create link{/if}</button></div>
	</div>
</Sheet>
