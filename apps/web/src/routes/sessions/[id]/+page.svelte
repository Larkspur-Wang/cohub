<script lang="ts">
  ...
  import { abortSession, getSessionStreamUrl, sendSessionMessage, type SessionRecord } from '$lib/api';
  ...
  function upsertTool(tool: ToolState) {
    const index = timeline.findIndex((item) => item.kind === 'tool' && item.tool.id === tool.id);
  ...
  function handleAgentEvent(payload: SessionEventPayload) {
    ...
    if (type === 'message_update') {
      ...
      if (deltaType === 'text_delta') {
        const current = timeline.find(
          (item) => item.kind === 'message' && item.message.id === assistantStreamingId
        );
      ...
      if (deltaType === 'thinking_delta') {
        const current = timeline.find(
          (item) => item.kind === 'message' && item.message.id === thinkingStreamingId
        );
        const deltaText = typeof delta?.delta === 'string' ? delta.delta : '';
        const nextText = `${current?.kind === 'message' ? current.message.text : ''}${deltaText}`;
        upsertMessage({
    ...
    if (type === 'message_end') {
      ...
      finalizeStreamingMessage(thinkingStreamingId, thinkingText
        ? {
            id: thinkingStreamingId,
            role: 'system',
            title: 'Thinking',
      ...
      finalizeStreamingMessage(assistantStreamingId, assistantText
        ? {
            id: assistantStreamingId,
            role: 'assistant',
            text: assistantText
    ...
    if (type === 'tool_execution_start') {
      const toolCallId = typeof payload.toolCallId === 'string' ? payload.toolCallId : crypto.randomUUID();
    ...
    if (type === 'tool_execution_update') {
      ...
      const output = extractTextContent(partialResult) || renderToolPreview(toolName, args);
    ...
    if (type === 'tool_execution_end') {
      ...
      const output = extractTextContent(result) || renderToolPreview(toolName, args);
    ...
    if (type === 'error') {
      appendMessage({
        role: 'error',
        title: 'Error',
        text: typeof payload.error === 'string' ? payload.error : stringifyUnknown(payload)
      });
      return;
    }
  ...
  onMount(() => {
    ...
    eventSource.addEventListener('message', (event) => {
      try {
        const payload = JSON.parse((event as MessageEvent).data) as SessionEventPayload;
