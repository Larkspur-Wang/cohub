...
<div class="flex-1 flex flex-col min-h-0 overflow-y-auto">
  ...
  <div class="h-10 flex items-center justify-between px-4 border-b border-border-primary shrink-0 bg-bg-primary">
    <div class="flex items-center gap-3 min-w-0">
      <button class="text-text-tertiary hover:text-text-primary transition-colors shrink-0" onclick={() => goto(`/runtimes/${data.runtime.id}`)}>
        <ArrowLeft class="w-4 h-4" />
      </button>
      <div class="w-[1px] h-4 bg-border-primary shrink-0"></div>
      <span class="text-xs font-medium text-text-secondary">Session Graph</span>
      <span class="text-[10px] text-text-placeholder truncate font-mono">{data.runtime.title || data.runtime.id}</span>
    </div>
  </div>
      ...
      {#each data.sessions.filter((session) => !session.parentSessionId) as root, rootIndex (root.id)}
        <div class="rounded-lg border border-border-primary bg-bg-surface p-4">
          <div class="flex items-center gap-2 mb-1">
            ...
            <span class="text-sm font-medium text-text-primary">{getSessionTitle(root, rootIndex)}</span>
          </div>
          <div class="text-[10px] text-text-placeholder font-mono ml-3.5">{root.id}</div>
          <div class="text-[10px] text-text-placeholder ml-3.5 mt-1">depth: {root.forkDepth ?? 0}</div>

          {#if (byParent.get(root.id)?.length ?? 0) > 0}
            <div class="mt-3 ml-3 space-y-2 border-l border-border-primary pl-4">
              {#each byParent.get(root.id) ?? [] as child, childIndex (child.id)}
                <div class="rounded-md border border-border-subtle bg-bg-code p-3">
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    <span class="text-xs text-text-secondary">{getSessionTitle(child, childIndex)}</span>
                  </div>
                  <div class="text-[10px] text-text-placeholder font-mono ml-3.5">{child.id}</div>
                  <div class="text-[10px] text-text-placeholder ml-3.5 mt-1">
                    forked from: {data.messagePreviewById[child.forkedFromMessageId ?? ""] ?? child.forkedFromMessageId ?? "unknown"}
                  </div>
                  <div class="text-[10px] text-text-placeholder ml-3.5">depth: {child.forkDepth ?? 0}</div>
                </div>
              {/each}
            </div>
          {/if}
