					...
					<div class="space-y-1">
						{#each members as member (member.userId)}
							<div class="group flex items-center gap-2 rounded-[5px] bg-bg-primary px-3 py-2">
								...
								<button type="button" onclick={() => { void removeMember(member.userId); }} disabled={removingMemberUserId === member.userId} title="Remove member" class="rounded-sm p-1 text-text-tertiary opacity-0 transition-colors hover:bg-bg-hover hover:text-error-soft group-hover:opacity-100 disabled:opacity-50">{#if removingMemberUserId === member.userId}<Loader2 class="w-3 h-3 animate-spin" />{:else}<X class="w-3 h-3" />{/if}</button>
							</div>
						{:else}
