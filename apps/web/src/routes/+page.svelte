...
onMount(async () => {
  ...
  try {
    ...
    recentSpaces = (spaces as SpaceRecord[])
      ...
      .map((space) => ({
        id: space.id,
        title: space.name || space.title || space.id.slice(0, 12),
      }));
  } catch (error) {
        ...
        <div class="mt-8">
          ...
          <div class="rounded-md border border-border-subtle overflow-hidden">
            {#each recentSpaces as space, i}
              ...
              >
                <div class="w-[6px] h-[6px] rounded-full shrink-0 bg-status-running"></div>
                <span class="flex-1 text-[13px] text-text-primary truncate">{space.title}</span>
                <span class="text-[11px] text-text-tertiary">Space</span>
