...
function parseTemplateFromText(raw: string, filePath: string, scope: PromptTemplateScope): PromptTemplate {
  ...
  return {
    name,
    description: attributes.description?.trim() || firstLine?.trim().slice(0, 80) || name,
...
async function loadCachedPrompts(input: { redisKey: string; dir: string; scope: PromptTemplateScope; allowMissing: boolean }): Promise<PromptTemplatesConfig | null> {
...
let service: PromptTemplateService | null = null;
export function getPromptTemplateService(): PromptTemplateService {
  service ??= { expand: expandPromptTemplate };
  return service;
}
