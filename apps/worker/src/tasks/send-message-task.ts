...
import { getPromptAuthScopes, type PromptAccessMode, type PromptAuthContext, type SubmitSessionPromptContext } from "@cohub/core/sessions";
...
const sendMessageHandler = async (job: import("bullmq").Job, context?: { taskRunId: string }) => {
  ...
  const { content, sessionId, title, source: payloadSource, model, provider, clientMessageId, generationPolicy, accessMode, intent, labelIds, auth } = (payload.data ?? {}) as {
